...
public:
	...
	Vec2(const Vec3& copyFrom);
	Vec2(const IntVec2& copyFrom);
	explicit Vec2( float initialX, float initialY );		// explicit constructor (from x, y)

	//A2 code below
	static Vec2 const MakeFromPolarRadians(float orientationRadian, float length=1.0f);
	static Vec2 const MakeFromPolarDegrees(float orientationDegree, float length = 1.0f);
	...
	void SetPolarRadians(float newPolarRadians,float newLength);
	void SetPolarDegrees(float newPolarDegrees, float newLength);
	...
	Vec2 const GetReflected(Vec2 const& surface)const;
	void Reflect(Vec2 const& surface);
