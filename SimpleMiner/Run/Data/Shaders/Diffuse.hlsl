...
v2p_t VertexMain(vs_input_t input)
{
    ...
    v2p.worldBitangent = worldPosition; //worldBitangent;
...
float4 PixelMain(v2p_t input) : SV_Target0
{
    ...
    float dist = length(CameraPosition - input.worldBitangent);
    
    float fogFraction = (dist - FogNearDistance) / (FogFarDistance - FogNearDistance);
    ...
    vertexColor = vertexColor * SkyColor;
    vertexColor.w *= 1 - fogFraction;
